...
- **same-error 判定:** `gh run view --log-failed` の出力（stdout）から ANSI エスケープシーケンスを除去し、`error` / `Error` / `failed` のいずれかを含む最終行を抽出する。この文字列をキーとして items 1-3 間で完全一致比較してグループ化する（タイムスタンプは比較対象に含む。たまたま不一致なら別グループ扱いとする）
