...
`status` は worker では最大 `merge-ready`（レビュー通過・未マージ）。`merged` / `auto-merge enabled` は親が Phase Final-4 で確定するため worker は返さない（後方互換のため親はこれらの値を受理はするが、通常の worker 戻り値には現れない）。
...
| ケース | 扱い |
|---|---|
| 対象ファイルを別 PR-B が編集済みで畳み込みが衝突 | 導入元 PR への畳み込みを諦め、**専用 reconciliation PR**（全 tip に stacked）へフォールバック |
...
1. `gh pr merge <上流> --squash --delete-branch`
...
3. auto-merge がリポジトリで無効 / branch protection 等で即マージ不可 → その PR を `merge-ready` として残置し、それに依存する下流を `skipped` にして warning を出す
4. 各 PR のマージ完了を `gh pr view --json mergedAt,state` で確認してから次へ進む（squash merge の反映を待つ）
...
`cross-cutting:` 行は Phase Final-3 で reconciliation により解消した gap 数と畳み込み先 PR を `<N> gaps resolved (folded into ...)` 形式で表示する。gap が 0 件なら `cross-cutting: none`。fail-soft で残った gap があれば `<N> resolved, <M> unresolved (warning)` とし、warning ブロックに未解決分と手動対応の推奨を出す。
