...
- **(b) debounce**: doesn't deny on a single over-threshold reading — only denies once it's exceeded for **N consecutive reads** (default 2, overridable via `USAGE_GUARD_DEBOUNCE_COUNT`). Even a single under-threshold reading in between resets the consecutive count. The consecutive count and the most recent good reading are kept in `~/.claude/usage-guard/hook-state.json`.
