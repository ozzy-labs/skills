...
決定論（引数の解析・target 展開・DAG/wave 構築・循環検出・並列度/review モード解決 / worker 戻り値の集約・レポート整形）は同梱の 2 つのエンジンが担う（[ADR-0028](https://github.com/ozzy-labs/handbook/blob/main/adr/0028-skills-architecture-engine-judgment-policy-catalog.md) R1、先行例 `health-check.mjs` / `topics.mjs` / `review.mjs`）:

- **`drive-plan.mjs`**: 入力（引数文字列＋任意で target メタ）→ wave 構成 JSON。target 展開・`->` 依存解析・DAG 構築・topological wave 分割・循環依存検出・モード分岐・並列度/review モード解決を担う。
- **`drive-report.mjs`**: worker 戻り値 JSON の集約 → 単一モード Phase 5 完了報告 / オーケストレーション Phase Final-6 集約レポートの整形。review 集計（by_axis / total）・cross-cutting gaps 表示・`再開: /drive <元の引数>` 行の出力判定を担う。

本 SKILL.md は判断層 — **いつエンジンを呼ぶか・各 Phase の意味・review loop の観点別終了判定・cross-cutting の畳み込み判断・どこで人に確認するか** — に絞る。挙動契約（Phase 1-5 / Phase Final-1〜6 のセマンティクス、失敗 semantics、`--merge` / `--review` / `--concurrency` の意味）は本ファイルが SSOT。

## 入力解析

引数の解析は `drive-plan.mjs` が担う（本 SKILL.md と同じディレクトリ。Claude Code では `~/.claude/skills/drive/drive-plan.mjs`、dogfood は `<repo>/.claude/skills/drive/drive-plan.mjs`）:
...
- **target 展開:** 単一 `#42` / `42`、カンマ列 `#1,#2`、範囲 `#3-5` → `#3,#4,#5`、空白列 `#1 #2`、混合 `#1,#3-5`。いずれにも該当しない引数はテキスト指示として扱う（target は単一）。
- **明示依存記法 `->`:** `#1,#2 -> #3`（#1・#2 並列 → #3 は両者完了後）、`#1 -> #2 -> #3`（完全直列）。
- **DAG 構築:** 明示依存（最優先）＋ PR base↔head 照合（stacked）＋ issue 本文 "depends on #X" の best-effort（`--meta-file` で issue/PR メタを渡した場合）。
- **wave 分割 / 循環検出:** topological levels に分割。循環依存があれば `error` + `cycle` を返す（→ 報告して中断）。
- **モード分岐:** target 1 件かつ依存なし → 単一モード、2 件以上または依存あり → オーケストレーションモード。
- **並列度 / review モード:** 既定 `min(4, タスク数)`、`--concurrency` 上書き（>8 は警告のみ）。オーケストレーションでは `--review=quick` を強制（`final-deep`/`deep` は警告して降格）。

エンジン出力（`--json`）の `resume_command` は「引数復元（再開コマンド）」規約に従って組み立て済み。

### オプション

- `--merge`: 自動マージを試行する。マージは中央 autonomy policy の `irreversible` gate に従う（下記「マージと autonomy policy」参照）。**`--merge` フラグ自体が明示 opt-in** — ユーザーが自律マージを委任済みとみなし、gate を `proceed`（確認なしでマージ）に上書きする
...
### マージと autonomy policy（irreversible gate）

マージ（単一モード Phase 4 / オーケストレーション Phase Final-4）は **不可逆・破壊的アクション**。個別の承認ゲートを prose にハードコードせず、中央 autonomy policy（`policy` skill が定義する 3 クラス・gate 語彙の SSOT。[ADR-0028](https://github.com/ozzy-labs/handbook/blob/main/adr/0028-skills-architecture-engine-judgment-policy-catalog.md) R3、[#181](https://github.com/ozzy-labs/skills/issues/181)）に従う。分類とゼロコンフィグ既定:

| 本 skill のアクション | クラス | policy 参照 | ゼロコンフィグ既定 gate |
|---|---|---|---|
| PR マージ（`gh pr merge`） | `irreversible` | `--action=merge` | `ask`（マージ前に明示承認） |
...
- **既定は `ask`:** `--merge` 未指定時、マージは gate=`ask` に従い**人へ確認**してから実行する（単一モードは Phase 4、オーケストレーションは Phase Final-4 の「完了後」確認）。
- **`--merge` は明示 opt-in で `proceed` に上書き:** ユーザーが `--merge` を付けた時点で自律マージを委任済みとみなし、gate を `proceed`（確認なしでマージ + audit trail）に上書きする。これは implement の `reversible-local`=`proceed`（drive 配下では計画承認をスキップ）と同じ「drive は自律実行を委任されている」原則の延長。
- **policy 不在でも壊れない（fail-safe）:** `policy-read.mjs` は fail-safe 設計で、読めない・不正な値は必ず厳しい側（`ask`）へ倒す。`policy` skill 未配置の環境では既定 gate（`irreversible`=`ask`）を直接適用する。`--merge` の明示 opt-in はこの場合も有効。
- **アーキテクチャ強制（Claude Code）と `--merge` の通し方:** `gh pr merge` の実行エンジン側 gate（PreToolUse `policy-hook.mjs`。`npx @ozzylabs/skills hooks add policy` で配線）が張られている環境では、gate=`ask`（既定）の merge は hook が deny（exit 2）する。**`--merge` は prose 層で merge を `proceed` に上書きする明示 opt-in なので、実際に `gh pr merge` を実行する際は `POLICY_GUARD_PROCEED=merge` を export して実行する**（hook はこの pre-authorized action を再ゲートせず allow する。[#195](https://github.com/ozzy-labs/skills/issues/195) gap 解消）。これにより hook 配線環境でも drive 自身の正当なマージが強制網に阻まれない。hook 未配線の環境では無害な env 前置きになるだけ。同様に、`--merge` 未指定時の「完了後」確認で人がマージを承認した場合も、その 1 コマンドは `POLICY_GUARD_PROCEED=merge` を前置きして実行する（AskUserQuestion による明示承認が `ask` gate の要求を満たしているため）。
...
ship スキルのワークフロー（verify → commit → PR 作成）を実行する。完了報告・次のアクション確認は無視する。
...
**中断条件:** verify（検証）が失敗し、修正できない場合 → エラーを報告して中断
...
3. **修正:** 未達観点の Critical および Warning の指摘事項のみを修正する。Info は修正しない（報告のみ）。修正後、verify → commit → push を実行し、1 に戻る
...
マージは中央 policy の `irreversible` gate に従う（「マージと autonomy policy」参照。既定 `ask`）。以下は **単一モードを直接（worker としてではなく）実行し、かつ `--merge` 指定時**にのみ実行する（`--merge` は gate を `proceed` に上書きする明示 opt-in）:

1. **Auto-merge の有効化:** `POLICY_GUARD_PROCEED=merge gh pr merge --auto --squash --delete-branch` を実行する（`POLICY_GUARD_PROCEED=merge` は `--merge` の proceed 上書きを PreToolUse `policy-hook.mjs` に伝える。「マージと autonomy policy」参照）
2. **成否の確認:**
...
完了報告の整形は `drive-report.mjs single` が担う。worker/単一モードの戻り値 JSON（`target` / `title` / `branch` / `pr_url` / `status` / `review` / `original_args`）を stdin または `--input` で渡すと、次の整形を返す（by_axis 集計・resume 行の出力判定を含む）:
...
`再開:` 行は状態が `failed` または `merge-ready` のとき**必ず**出力する（発見可能性のため。resume が効くことを SKILL.md を読まずに知れるようにする）。引数は「引数復元（再開コマンド）」の規約に従って復元し、`再開: /drive <元の引数>` の 1 行で示す。再実行すると冪等 resume が既存 PR / ブランチを検出し、Phase 3 から続きを再開する。状態が `merged` / `auto-merge enabled` で完結した場合は表示しない。この出力判定と引数復元は `drive-report.mjs` が実装する。

## オーケストレーションモード

### Phase 0: 入力展開と DAG 構築

target 展開・DAG 構築・wave 分割・循環検出は `drive-plan.mjs` が担う（判断層は GitHub からのメタ取得とエンジン呼び出しのみ）:

1. 各 target について GitHub からメタを取得する（DAG の PR base 照合 / issue 本文検出に使う）:
   ...
   - 取得したメタを `{"#N": {kind,title,body,baseRefName,headRefName}, ...}` の JSON にまとめ、一時ファイルへ書き出す
2. `drive-plan.mjs "<元の引数>" --meta-file <path> --json` を実行する。エンジンが返す `waves` / `deps` / `mode` / `concurrency` / `review` / `cycle` を採用する:
   - **DAG:** 明示依存 `->`（最優先）＋ PR base↔head 照合（stacked）＋ issue 本文 "depends on #X" / "blocked by #X" / "after #X" の best-effort（取りこぼしはエラーにせず並列扱いにフォールバック）
   - **循環依存:** エンジンが `error: "circular dependency detected"` + `cycle` を返したらエラー報告して中断する
3. `drive-plan.mjs`（`--json` なし）の出力をそのまま提示する。整形はエンジンが担う（再整形しない）:
...
worker は self-merge しないため、実際のマージはここで親が一元管理する。マージは中央 policy の `irreversible` gate に従う（「マージと autonomy policy」参照。既定 `ask`）。`--merge` 指定時に実行する（`--merge` は gate を `proceed` に上書きする明示 opt-in。未指定時は既定 `ask` に従い下記「完了後」の確認を経る）。

topological order（依存元→依存先）で各 PR をマージする:

1. `POLICY_GUARD_PROCEED=merge gh pr merge <上流> --squash`（**remote-only マージ。`--delete-branch` は付けない** — この時点で worker 作業コピーが当該 branch を握って残存しているため、local branch 削除は `fatal: '<branch>' is already used by worktree` で失敗する。remote/local branch と作業コピーの削除は Final-5 cleanup に一本化する。`POLICY_GUARD_PROCEED=merge` は `--merge` の proceed 上書きを PreToolUse `policy-hook.mjs` に伝える。「マージと autonomy policy」参照）
...
集約レポートの整形は `drive-report.mjs aggregate` が担う。worker 戻り値配列と Final-1〜5 で集めたメタ（`{results, targets_order, original_args, cross_cutting, cleanup, integrity_warnings}`）を stdin または `--input` で渡すと、status 別カウント・review 集計・cross-cutting/cleanup 行・resume 行の出力判定を整形して返す:
...
- マージは中央 autonomy policy の `irreversible` gate に従う（既定 `ask`。「マージと autonomy policy」参照）。デフォルトでは行わず、`--merge` 指定時のみ実行する（`--merge` は gate を `proceed` に上書きする明示 opt-in。単一モードは Phase 4 で Auto-merge、オーケストレーションは Phase Final-4 で親が依存順に一元マージ）
