...
- **same-error 判定:** `gh run view --log-failed` の出力（stdout）から (1) ANSI エスケープシーケンス、(2) 行頭の ISO 8601 タイムスタンプ（`YYYY-MM-DDTHH:MM:SS.fffZ` 形式、後続スペース 1 個を含む）を除去し、`error` / `Error` / `failed` のいずれかを含む最終行を抽出する。この文字列をキーとして items 1-3 間で完全一致比較してグループ化する（タイムスタンプ除去後も差分が残る場合は別グループ扱い。決定論性のため fuzzy matching はしない）
