...
- **アーキテクチャ強制（Claude Code）:** `gh pr merge` の実行エンジン側 gate（PreToolUse `policy-hook.mjs`）は [#181](https://github.com/ozzy-labs/skills/issues/181)-PR4 の範囲。本 skill は prose 判断層で policy を引く。
