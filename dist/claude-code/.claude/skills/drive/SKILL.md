...
3. `ok: false` (either budget exceeds the threshold) → defer to usage-guard's wait-loop. A post-reset buffer (`resume_buffer_seconds`, default +300 seconds) is folded into `wait_seconds`, and the wait extends to `resets_at + buffer` (avoiding a re-bounce from re-entering right at the reset boundary). **Execute the engine's `resume_plan` (#212) rather than re-deriving the trigger by hand**: `usage-check.mjs` emits `resume_plan { trigger, wait_seconds, fire_at }`, and orchestration checkpoints pass `--context orchestration` so the plan prefers a durable one-shot (mapping in usage-guard SKILL.md §"Resume plan"). The `trigger` cases:
   - `schedule-wakeup` (in-session, wait ≤1h) → set up a heartbeat with `ScheduleWakeup(min(wait_seconds, 3600))` and **wait** (no re-entry and no budget consumption while waiting). If `wait_seconds` exceeds 3600, re-check in multiple installments.
   - `cron-oneshot` (non-`/loop` orchestration, wait >1h, needing restart resilience) → set a `CronCreate` (`recurring: false`, durable) to **`fire_at`** (`= resets_at + resume_buffer_seconds`), and re-submit the continuation command when it fires (a single wall-clock shot, restart-resilient; the one-shot auto-deletes after firing). Since this being ON by default makes this path (>1h, non-`/loop`) easy to hit, this is preferred over `ScheduleWakeup`. See usage-guard SKILL.md §Lightweight wait-loop, "Choosing the resume trigger", for details.
   - `cron-routine` (durable, survives a 100% hard-interrupt — #213) → arm a **cloud Routine** at `fire_at`. See "Durable pre-arm" below; validate `firesAfterReset(plan, resets_at)` first, and fall back to `cron-oneshot` + a warning where `/schedule` is unavailable.
...
#### Durable pre-arm (hard-interrupt resilience, #213/#217)

A reactive pause (steps above) keeps the session alive so an **in-session** trigger (`ScheduleWakeup` / a session-scoped `CronCreate`) can fire. But if the boundary checkpoint is bypassed and a worker burns all the way to **100%**, the session **hard-interrupts** and those in-session triggers die with it — the exact "stopped and never resumed" failure. Only a **cloud Routine** (scheduled deployment) survives a hard-interrupt.

For orchestration / long runs with hard-interrupt risk, **pre-arm a durable resume at run start** (not reactively):

1. Before launching the first wave, run `usage-check.mjs --context durable` to obtain a `resume_plan` with `trigger: "cron-routine"` and `fire_at`.
2. **Assert `firesAfterReset(plan, resets_at)`** (a Routine firing at/before the reset is rejected with `session_rate_limited_error`; `fire_at = resets_at + buffer` already satisfies this — the guard catches a `resume_buffer=0` / clock-skew edge).
3. Arm a **cloud Routine** at `fire_at` whose prompt is the continuation command `/drive <元の引数>` (idempotent — drive resumes from the existing PR/branch/cached workers, so a spurious fire is a harmless no-op).
4. On a clean finish (limit never hit), the one-shot Routine either no-ops on its idempotent continuation or is cancelled.
5. **Fallback**: where the host `/schedule` (Routines) is unavailable, fall back to the in-session `cron-oneshot` path + a degradation warning — a durable path that cannot be armed must not silently drop the resume.

This composes with the two existing layers (headroom-aware boundary pause + the `#123` PreToolUse ceiling): the pre-armed Routine is the **last-resort net** that recovers even a genuine hard-interrupt.
