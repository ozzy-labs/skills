...
- `/health` — リポジトリ状態と skill catalog 整合性を `health-check.mjs` エンジンで 16 領域確認し、固定語彙の推奨アクションを提示（`--deep` で `要確認` 項目を read-only 追加調査、`--fix` で安全語彙 — prune / delete / fetch と `--deep` 昇格 drop — のみ単一確認付きで実行、`--yes` でスキップ）。既定は read-only。policy 連携（gate 差し替え）は [#181](https://github.com/ozzy-labs/skills/issues/181)-PR3 予定
