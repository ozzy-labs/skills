...
- `/release` — release-please PR を検証 → ゲート付きマージ → publish 監視する上流〜配布 skill（[#178](https://github.com/ozzy-labs/skills/issues/178)）。release-please PR を検出（`gh pr list --author app/release-please --state open`。0 件なら「リリース対象なし」+ draft release 有無を併記、health 領域 14）→ 固定チェックリスト（version bump と含有 commit type の SemVer 整合 — feat→minor / fix→patch / `!` or BREAKING CHANGE→major、CHANGELOG 整合、CI 全 green）で検証 → **既定は承認ゲート**（外部可視・実質不可逆のため維持。merge は中央 autonomy policy の `irreversible` gate に従う — `--action=merge`・ゼロコンフィグ既定 `ask`）→ `gh pr merge --squash` → publish workflow を polling 監視（30s 間隔・上限 20 分）→ `npm view <pkg> version` で反映確認（npm 配布リポのみ）。`--auto` は**全検証 pass 時のみ**ゲート skip（検証 fail 時は `--auto` でも停止）。失敗時は `gh run view --log-failed` の要約 + よくある原因（Trusted Publisher / provenance / permissions）を案内（自動修正はスコープ外）。npm publish は OIDC Trusted Publishers（`NPM_TOKEN` 不使用）。publish workflow なしリポは merge + tag / Release 確認で完了。単一リポのみ
...
- `/lessons-triage` — セッション教訓 queue（`~/.agents/lessons/queue.jsonl`）を消化し、User Skills 改善の教訓を承認制で ozzy-labs/skills へ issue 起票（HITL、起票のみ）。observability ループの**反映層**: `skill-metrics` の rollup（`/skill-metrics --snapshot`）を metrics-primed な優先付け起点として受け取り、起票する `[lessons]` issue を **backlog ポインタ**（rollup 件数を根拠に添える優先 index。fix は運ばず診断・修正はローカル）として構成。反映（送信）は常に opt-in・HITL、rollup 引用は metadata のみ（逐語ログ・payload・raw path なし）。起票 `[lessons]` issue は `auto-ok` ラベル運用（**人間のみ付与**・本 skill は自動付与しない）で `/backlog --auto` の消化対象に接続し、reflect→consume を閉じる（人間の境界制御は「起票承認」+「ラベル付与」の 2 点＝HATL）。トレンド比較は `skill-metrics`、週次 routine は README「Observability」/ `docs/observability-routine.md` の責務（[#162](https://github.com/ozzy-labs/skills/issues/162)-PR3 + [#184](https://github.com/ozzy-labs/skills/issues/184)）
