...
- `.agents/skills/skill-metrics/` — observability イベントログを read-only 集計する user-invocable skill（全 adapter・shipped）。`skill-metrics.mjs`（集計エンジン・小 n ガード・任意 snapshot）を同梱。送信はせず、反映は `lessons-triage` の責務。詳細は README.md「Observability」を参照
