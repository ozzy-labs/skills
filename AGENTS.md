...
- `.agents/skills/skill-metrics/` — observability イベントログを read-only 集計する user-invocable skill（全 adapter・shipped）。`skill-metrics.mjs`（集計エンジン・小 n ガード・任意 snapshot・前週比 trend 比較）を同梱。送信はせず、反映は `lessons-triage` の責務。詳細は README.md「Observability」を参照
- `.agents/skills/skill-observability/` — skill 改善ループの計測層を支える被参照 companion（`adapters: claude-code`）。`event.schema.json`（イベント契約の単一 SSOT）、`obs-emit.mjs`（fail-open な emit substrate）、`obs-derive.mjs`（transcript から発火を導出する SessionEnd capture hook・手動 wiring）を同梱する。イベントは runtime に `~/.agents/observability/events.jsonl`（HOME-anchored・追記専用・skills dir 外）へ書かれる。詳細は README.md「Observability」を参照
- `.agents/skills/policy/` — 中央 autonomy policy（ADR-0028 R3）の被参照 companion（全 adapter・`user-invocable: false`）。`policy.schema.json`（policy 契約の単一 SSOT・`additionalProperties: false`）と `policy-read.mjs`（全 adapter 共通の読取 CLI。`~/.agents/policy.yaml` + repo の `.agents/policy.yaml` をマージして有効値を JSON で返す）を同梱する。信頼できない値は厳しい側（`ask`）へ倒す fail-safe。詳細は README.md skill 表を参照
