...
| `health` | リポジトリ状態と skill catalog 整合性を `health-check.mjs` エンジンで 16 領域確認し、推奨アクションを inline 表示（`--deep` で `要確認` 項目を追加調査、`--fix` で安全語彙 — prune / delete / fetch と `--deep` 昇格 drop — のみ単一確認付きで実行、`--yes` でスキップ）。既定は read-only |
