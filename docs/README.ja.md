...
合計 20 件: OzzyLabs 全リポジトリ共通の 15 件 (reference-only な `policy` companion を含む)、Claude Code 専用 2 件 (`usage-guard` / `skill-observability`)、内部運用専用 3 件 (`health` / `topics` / `phase-issue`) をバンドル。（旧 project-scope レイアウトを片付ける `migrate` subcommand は撤去済み — [#151](https://github.com/ozzy-labs/skills/issues/151)）。
...
| スキル | 用途 |
| --- | --- |
| `backlog` | open issue を `backlog.mjs` エンジンで収集し drive へ接続。依存抽出は drive エンジンの規則（`drive-plan.mjs` の `detectBodyDeps` / `topoWaves`、単一 SSOT・再掲しない）を再利用し、固定語彙の優先度規則（(a) blocker / (b) milestone 期限昇順 / (c) `priority:high` ラベル / (d) updatedAt 古い順・決定論的 tie-break）で候補を並べ、drive 引数形式（`#12,#15 -> #18`）で出力。既定は提示のみ、`--drive[=N]` は上位 N 件 + 依存閉包を確認後 drive へ、`--auto` は無確認だが `auto-ok` ラベル付き issue のみ対象（HATL）で中央 autonomy policy（drive 起動 = `externally-visible` → batch-confirm）に従う。単一リポのみ |
| `ci-fix` | 失敗した CI run のログを収集してコンテキストを整形し `/drive` へ接続する薄い wrapper。入力解決（明示 run id > 明示 branch の最新 failure > 現在ブランチの最新 failure、`gh run list --branch <b> --status failure --limit 1`）→ flaky 判定（`gh run rerun --failed` 1 回 + polling 30s 間隔・上限 15 分、`--no-rerun` で skip）→ ログ抽出（`gh run view --log-failed`、ANSI 除去 + エラー行抽出 regex は health の same-error 判定（`health-check.mjs` の `extractCiErrorKey`）と同一で sync assertion で強制）→ 指示テキスト組み立て → `/drive` 起動。`--dry-run` は指示テキストのみ出力（rerun も drive 起動もしない）、`--auto` は起動前の確認を skip。main ブランチの failure（merged コードの破損）は優先度高としてレポート冒頭で明示 |
| `commit` | 変更をステージし Conventional Commits でコミット |
| `commit-conventions` | コミット / ブランチ / PR の命名規則 |
| `deps` | automation PR（renovate / dependabot）の policy-based triage を `deps.mjs` エンジンで実行。open な bot PR を列挙（author 判定は health 領域 15 と同一 — `*[bot]` / `app/*`、release-please は除外＝ `/release` の責務）し、semver 区分（PR タイトル / branch / manifest diff、grouped は最大 bump）・CI 状態（`gh pr checks`）・lockfile 整合・peer / engines 変更で固定語彙判定する: patch/minor + CI green + lockfile 整合 + peer/engines なし → auto-merge 候補、major / CI red / pending / no-checks / 区分不能 / lockfile drift / peer / engines → 要確認。`--dry-run` は判定のみ（`--auto` より優先）、`--auto` は確認なし merge。merge（`gh pr merge --squash`）は不可逆で中央 autonomy policy に従う（`--action=merge`・ゼロコンフィグ既定 `ask`）、`--auto` が明示 opt-out。単一リポのみ |
| `drive` | Issue → merge-ready な PR まで自律駆動するループ。Claude Code ではオーケストレーションを**ハーネスネイティブな Dynamic Workflow** で実行（ADR-0028 R2）: `drive-plan.mjs` の wave 計画を script の `args` に渡し、各 wave は `agent({ isolation: 'worktree' })` worker の `parallel()` バリア、中断再開は Workflow の journal resume（`resumeFromRunId`。`gh pr list` polling ではない）。Dynamic Workflows 不可の環境では Agent tool 方式に fallback |
...
| `lessons-triage` | セッション教訓 queue（`~/.agents/lessons/queue.jsonl`、dotfiles の `lesson-capture.sh` hook が蓄積）を消化し、transcript から User Skills 改善の教訓を抽出して承認分のみ ozzy-labs/skills へ issue 起票。HITL — issue 起票は中央 autonomy policy の externally-visible gate（batch-confirm: 全教訓を 1 回一括承認）に従う、リポ編集・auto-apply なし。起票 `[lessons]` issue は人間が `auto-ok` ラベル（人間のみ付与・本 skill は付けない）を付けると `backlog --auto` に接続され、ループの reflect→consume を閉じる |
...
| `release` | release-please PR を検証 → ゲート付きマージ → publish 監視。検出（`gh pr list --author app/release-please --state open`。0 件 → 「リリース対象なし」+ draft release 有無を併記、health 領域 14）→ 固定チェックリスト（version bump と含有 commit type の SemVer 整合 — feat→minor / fix→patch / `!` or BREAKING CHANGE→major、CHANGELOG 整合、CI 全 green）で検証し、既定は承認ゲート（外部可視・実質不可逆のため merge は中央 autonomy policy に従う: `--action=merge`・ゼロコンフィグ既定 `ask`）→ `gh pr merge --squash` → publish workflow を polling 監視（30s 間隔・上限 20 分）→ `npm view <pkg> version` で反映確認（npm 配布リポのみ）。`--auto` は全検証 pass 時のみゲート skip（fail 時は停止）。失敗時は `gh run view --log-failed` の要約 + よくある原因（Trusted Publisher / provenance / permissions）を案内。npm publish は OIDC Trusted Publishers（`NPM_TOKEN` 不使用）。publish workflow なしリポは merge + tag / Release 確認で完了。単一リポのみ |
...
| `skill-metrics` | observability イベントログ（`~/.agents/observability/events.jsonl`）を read-only 集計し、skill 別発火件数 + 注目イベント（fallback / HITL 却下 / loop 上限 / 中断）を提示。小 n ガードで分母が十分なときのみ率を出す。`--snapshot` は週次 baseline を書き出し前週と差分（`trend`: 前週比 delta・小 n ガード継承）。ローカル完結・送信なし（反映は `lessons-triage`）。下記「Observability」を参照 |
...
`skill-observability` はデータ駆動の skill 改善ループ（捕捉 → 集計 → 反映 → 消化）の計測基盤を敷く被参照 companion。3 つのアーティファクトを同梱する:
...
**`skill-metrics`** skill（shipped・全 adapter）がこのログを read-only 集計し、skill 別発火件数 + 注目イベントを提示する（小 n ガードで低頻度データに誤解を招く `1/1 = 100%` 率を出さない）。`--snapshot` は今週の rollup を `~/.agents/observability/snapshots/<YYYY-Www>.json` に書き出し、前週スナップショットと差分を取る — 前週比 delta の `trend` ブロック（発火数・摩擦シグナルの件数 delta は常に提示、率 delta は**両週とも**小 n ガードを超えたときのみ）。

ループは残り 2 つの skill で閉じる:

- **`lessons-triage`**（反映）は `skill-metrics` の rollup（`/skill-metrics --snapshot`）を *metrics-primed* な起点として読む。摩擦の多い skill の transcript を優先精読し、privacy 洗浄済みの **backlog ポインタ** `[lessons]` issue を、rollup の件数を根拠として起票する。issue は「fix を運ぶ器」ではなく**どこを見るか**を指す — 診断と fix-PR は transcript のあるローカルで行う。反映（issue 起票）は常に opt-in・HITL（中央 autonomy policy の `externally-visible` gate）。rollup 引用は metadata（件数・window）のみで、逐語トランスクリプト・payload・secret・raw repo 名/cwd/PR 生値を載せない。
- **`backlog`**（消化）はその open issue を集め、固定の優先度規則で並べ替えて `drive` に渡す — 優先 index を fix-PR に変える。`--auto` は人間が `auto-ok` ラベルを付けた issue のみ消化する。

全体ループ（各段が次段へ供給し、ship された fix は次 window で再計測される）:

```text
skill-observability  →  skill-metrics  →  lessons-triage  →  backlog  →  (drive → fix-PR)
     捕捉                  集計              反映              消化
   events.jsonl        件数 + notable      backlog ポインタ   優先 index
 (obs-derive hook)     (--snapshot + trend)  issue (HITL)       → /drive
        ▲                                                              │
        └───────────── fix を ship・次 window で再計測 ───────────────┘
```
...
> **Release PR には既定でない token が必要。** 既定の `GITHUB_TOKEN` で開かれた Release PR は CI をトリガしないため、必須 check `lint-and-build` が付かず `main-protection` ruleset に恒久的に `BLOCKED` される。`release.yaml` は `token: ${{ secrets.RELEASE_PLEASE_TOKEN || secrets.GITHUB_TOKEN }}` を渡す — `RELEASE_PLEASE_TOKEN`（GitHub App token 推奨、または fine-grained PAT）を設定すると Release PR が check を通せる。初回リリースは手動（npm 上にパッケージが無く Trusted Publisher を事前設定できないため）。詳細な runbook は [`docs/releasing.md`](releasing.md)。
