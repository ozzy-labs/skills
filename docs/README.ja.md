...
この契約の上に別 PR で構築する（[#162](https://github.com/ozzy-labs/skills/issues/162) で追跡）: outcome 導出（`gh`/`git` の merge ground truth + session→PR linkage）、`/skill-metrics` 集計（件数 + 注目イベント）、反映チャネル（privacy 洗浄済みロールアップを `lessons-triage` issue に HITL で反映）。
