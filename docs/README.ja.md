...
合計 19 件: OzzyLabs 全リポジトリ共通の 14 件 (reference-only な `policy` companion を含む)、Claude Code 専用 2 件 (`usage-guard` / `skill-observability`)、内部運用専用 3 件 (`health` / `topics` / `phase-issue`) をバンドル。（旧 project-scope レイアウトを片付ける `migrate` subcommand は撤去済み — [#151](https://github.com/ozzy-labs/skills/issues/151)）。
