...
オーケストレーションの worker 並列実行は **Workflow 方式が canonical（既定）**。Claude Code の Dynamic Workflows（決定論 script + journal resume + `agent()` 並列起動）にハーネスネイティブへ載せ、wave 実行・worker 分配・進捗観測・中断再開をランタイムに委ねる（[ADR-0028](https://github.com/ozzy-labs/handbook/blob/main/adr/0028-skills-architecture-engine-judgment-policy-catalog.md) R2）。**Workflow tool が利用不可な環境**（Dynamic Workflows 無効・`disableWorkflows` / 旧バージョン）でのみ、従来の Agent tool 方式（「subagent dispatch」節）に fallback する。

| | Workflow 方式（canonical） | Agent tool 方式（fallback） |
|---|---|---|
| 並列制御 | ランタイムが cap（最大 16）・キュー管理 | 手動 semaphore |
| worktree 隔離 | `agent({ isolation: 'worktree' })` | `Agent({ isolation: "worktree" })` |
| 戻り値検証 | `schema` で構造化検証（不一致は自動リトライ） | JSON 自由記述を親が parse |
| 進捗監視 | `/workflows` UI + `log()` | `gh pr list` 30 秒 polling |
| 中断再開 | journal resume（`resumeFromRunId`・完了 worker はキャッシュ復元） | 手動再実行（drive 冪等 resume で PR 検出） |

### Workflow 方式によるオーケストレーション（canonical）
...
スクリプト内の `workerPrompt(target)` には以下を必ず含める（上のスクリプトに骨子を埋め込み済み。Agent tool 方式の「subagent dispatch」と同一の制約。ランタイムの worktree 隔離は cleanup を肩代わりするが、worker の git 操作自体は防がないため prompt 制約は省略不可）:
...
- 戻り値 JSON contract（`status` は最大 `merge-ready` / `final_head_state` / `cross_cutting_gaps` 含む。上の `WORKER_SCHEMA` と一致）

Workflow 方式固有の注意:

- **スクリプト内で `Date.now()` / `Math.random()` / 引数なし `new Date()` は使えない**（resume 決定性のためランタイムが throw する）。タイムスタンプが必要なら `args` で渡す
- **観測性は `/workflows` UI と `log()` が担う**（`p` pause / `x` stop / agent ごとの token 監視）。Agent tool 方式の `gh pr list` 30 秒 polling は**不要**（Workflow 方式では PR 検出 polling をしない）
...
- **中断再開は Workflow の journal resume**: `Workflow({scriptPath, resumeFromRunId})` で再入すると、変更されていない `agent()` 呼び出し（完了済み worker）は journal / キャッシュから復元され、未完了 wave から続行する。これが従来の「`gh pr list` 30 秒 polling で PR を検出し手動再実行」を置き換える主機構。drive 冪等 resume（既存 PR 検出）は二重の安全網として残る（worker 内部でも効く）
...
Phase 0 完了時に（Workflow 起動前・会話側で）wave 構成と target リストを表示する。Phase Final で集約レポートを出力する。進捗観測と中断再開の機構は方式ごとに異なる:

**Workflow 方式（canonical）:**

- 進捗は各 worker の `log()` 1 行出力と `/workflows` UI（`p` pause / `x` stop / agent ごとの token 監視）が担う。**`gh pr list` polling はしない**
- 中断再開は **Workflow の journal resume**: `Workflow({scriptPath, resumeFromRunId})` で再入すると、完了済み worker（変更されていない `agent()` 呼び出し）は journal / キャッシュから復元され、未完了 wave から続行する。従来の「wave 起動時刻から `gh pr list` を 30 秒 polling し PR 検出で resume」を置き換える
- 状態確定は worker 戻り値 JSON（`schema` 検証済み）が SSOT
...
- 中断再開は手動再実行（drive 冪等 resume が既存 PR / ブランチを検出して Phase 3 から続行）
...
- **`POLICY_GUARD_PROCEED=merge gh pr merge --squash` は `--delete-branch` を付けない**（worker worktree が当該 branch を握って残存中のため local branch 削除が worktree lock で失敗する。branch / worktree の削除は Final-5 cleanup に一本化）。`POLICY_GUARD_PROCEED=merge` は PreToolUse `policy-hook.mjs`（`hooks add policy` で配線）に `--merge` の proceed 上書きを伝え、gate=`ask` 由来の deny を回避する（canonical「マージと autonomy policy」/ [#195](https://github.com/ozzy-labs/skills/issues/195)）
...
2. **`--merge` 未指定時:** AskUserQuestion を呼び出す（`answers` パラメータは設定しない）
   - **「PR をマージする」** → `POLICY_GUARD_PROCEED=merge gh pr merge --squash --delete-branch` でマージを実行し、結果を報告する（AskUserQuestion の明示承認が `ask` gate を満たすため、PreToolUse `policy-hook.mjs` を通すよう `POLICY_GUARD_PROCEED=merge` を前置きする。canonical「マージと autonomy policy」参照）
   - **「追加の変更を行う」** → 終了する
