...
| `skill-metrics` | Read-only aggregator over the observability event log (`~/.agents/observability/events.jsonl`): per-skill invocation counts + notable friction events (fallback / HITL-reject / loop-cap / abort), with a min-n guard so rates are shown only when the denominator is large enough. `--snapshot` writes a weekly baseline and diffs it against last week (`trend`: week-over-week deltas, min-n guard inherited). Local-only — never sends anything; reflection (issue-filing) is `lessons-triage`'s job. See "Observability" below |
...
- **`backlog`** (consume) collects those open issues, orders them by its fixed priority rules, and hands them to `drive` — turning the priority index into fix-PRs. `--auto` consumes only issues a human has labelled `auto-ok`.
...
> **Release PRs need a non-default token.** A release PR opened by the default `GITHUB_TOKEN` does not trigger CI, so the required `lint-and-build` check never appears and the `main-protection` ruleset keeps it permanently `BLOCKED`. `release.yaml` passes `token: ${{ secrets.RELEASE_PLEASE_TOKEN || secrets.GITHUB_TOKEN }}` — set `RELEASE_PLEASE_TOKEN` to a fine-grained PAT, or mint a GitHub App token in-workflow (preferred), to make release PRs pass the check. The first release is done manually (npm has no package yet, so the Trusted Publisher can't be pre-set). Full runbook: [`docs/releasing.md`](docs/releasing.md).
