...
> **Release PRs need a non-default token.** A release PR opened by the default `GITHUB_TOKEN` does not trigger CI, so the required `lint-and-build` check never appears and the `main-protection` ruleset keeps it permanently `BLOCKED`. `release.yaml` passes `token: ${{ secrets.RELEASE_PLEASE_TOKEN || secrets.GITHUB_TOKEN }}` — set `RELEASE_PLEASE_TOKEN` (GitHub App token, preferred, or a fine-grained PAT) to make release PRs pass the check. The first release is done manually (npm has no package yet, so the Trusted Publisher can't be pre-set). Full runbook: [`docs/releasing.md`](docs/releasing.md).
